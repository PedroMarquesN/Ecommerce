...
const data = ref({
  labels: ['Beco Do Espeto', 'Pizza Place', 'Burger House', 'Sushi Spot'],
  datasets: [
    {
      label: 'Vendas',
      data: [2000, 1500, 3000, 2500],
      backgroundColor: ['#FF6347', '#1A56DB', '#FFD700', '#34D399'],
      borderColor: '#ffffff',
      borderWidth: 1,
    },
  ],
  ...
  <UserAdminLayout>
    <div class="max-w-5xl w-full bg-white rounded-lg md:ml-44 shadow dark:bg-gray-800 p-4 md:p-6">
      ...
      <div id="bar-chart" class="mb-4">
        ...
        <Bar :data="data" :options="options" class="chart-style" />
      </div>
    ...
    <div class="flex max-w-6x1 flex-nowrap justify-between mt-10 md:ml-36 mr-4">
      <Link :href="route('graphic.index')" type="button" class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Gráfico de Vendas</Link>
 
      <Link :href="route('graphic.pie')" type="button" class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Gráfico de Produtos</Link>
